...
L = 1; Nx = 2^4; x = linspace(0, L, Nx+1); hx = x(2) - x(1); %space grid
T = 1; Nt = 2^4; t = linspace(0, T, Nt+1); ht = t(2) - t(1); %time grid
...
FDto = BuildCNRhsDto(Nt, ht, Nx, A, yExact(x(2:end-1), t(1)), ...
...
norm(yDto - yExact(x(2:end-1), t(2:end)), "inf")
norm(lamDto - lamExact(x(2:end-1), t(2:end), nu), "inf")
norm(ht*(yDto - yExact(x(2:end-1), t(2:end))))
norm(ht*(lamDto - lamExact(x(2:end-1), t(2:end), nu)))
% 
% eyDto = [0.3104 0.1616 0.0852 0.0434 0.0219 0.0110];
% elamDto = [14.9163 15.8682 16.3372 16.5709 16.6876 16.7460];
% eyDto2 = [0.0324 0.0087 0.0023 5.8807e-04 1.4898e-04 3.7485e-05];
% elamDto2 = [1.3517 0.7274 0.3776 0.1924 0.0971 0.0488];
% k = 3:8; hk = 2.^(-k);
% loglog(hk, eyDto, hk, elamDto, hk, eyDto2, hk, elamDto2, hk, hk/hk(1), hk, (hk/hk(1)).^2);
...
norm(yOtd - yExact(x(2:end-1), t), "inf")
norm(lamOtd - lamExact(x(2:end-1), t, nu), "inf")
norm(ht*(yOtd - yExact(x(2:end-1), t)))
norm(ht*(lamOtd - lamExact(x(2:end-1), t, nu)))

% eyOtd = [0.2537 0.1188 0.0576 0.0284 0.0141 0.0070];
% elamOtd = [1.4783 0.7265 0.3608 0.1799 0.0898 0.0449];
% eyOtd2 = [0.0259 0.0062 0.0015 3.7515e-04 9.3349e-05 2.3283e-05];
% elamOtd2 = [0.1484 0.0358 0.0089 0.0022 5.5318e-04 1.3827e-04];
% k = 3:8; hk = 2.^(-k);
% loglog(hk, eyOtd, hk, elamOtd, hk, eyOtd2, hk, elamOtd2, hk, hk/hk(1), hk, (hk/hk(1)).^2);
%------------------------------%
...
function F = BuildCNRhsDto(N, dt, J, A, Y0, Yhat, gam)
%construct F1 to solve Y
F1 = zeros(N*(J-1), 1);
F1(1:J-1) = (speye(size(A)) - dt*A/2)*Y0;
